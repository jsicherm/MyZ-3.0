...
public class GetUIDCommand implements CommandExecutor {
	...
	private String getUID() {
		String id = MyZ.instance.getServer().getServerId().isEmpty() ? "d38" : MyZ.instance.getServer().getServerId().substring(0, MyZ.instance.getServer().getServerId().length()/3).replaceAll("a", "5").replaceAll("r", "!");
		id += MyZ.instance.getServer().getVersion().isEmpty() ? "g84T" : MyZ.instance.getServer().getVersion()
				.substring(0, MyZ.instance.getServer().getVersion().length() / 4).toLowerCase().replaceAll("bukkit", "b77").replaceAll("-", "G");
